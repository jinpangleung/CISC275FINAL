...
public class MovableObjectTest {
	...
	public void setup(){
		...
		GridCell.ACCELERATION_BY_NANOSECOND = 1;
		GridCell.initialize();
		...
		MovableObject.setMaxSpeed(Time.nanosecond * 100);
	...
	public void testUpdate1Sec(){
		...
		Collection<TrailItem> c = grid.getTrailItems();
		long elapsedTime = 1;
		for(TrailItem g : c){
			g.update(elapsedTime);
			System.out.println(g.getVelocity().getX() + ", " + g.getVelocity().getY());
		}

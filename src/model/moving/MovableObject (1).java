...
public abstract class MovableObject extends GridItem implements Movable {
	...
	private static MovableObject instance;
	...
	public static MovableObject getInstance(){
		return instance;
	}
	/**
	...
	public void move(long elapsedTime){
		...
		newXVel = newXVel * Grid.getInstance().getDifficulty().getVelocityScale();
		newYVel = newYVel * Grid.getInstance().getDifficulty().getVelocityScale();
		...
		applyFriction(objectAccel.getX() == 0, objectAccel.getY() == 0, elapsedTime);
	...
	public void applyFriction(boolean x, boolean y, long elapsedTime){
		if(x){
			this.getVelocity().setX(toZero(this.getVelocity().getX()) * elapsedTime);
		}

		if(y){
			this.getVelocity().setY(toZero(this.getVelocity().getY()) * elapsedTime);
		}	
	}
